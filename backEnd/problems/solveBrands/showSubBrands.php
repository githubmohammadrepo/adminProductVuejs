...
class ShowBrands {
  ...
  private function getAllSubBrands(int $brand_id,int $offfset = 0,int $count = 25):array{
    ...
    FROM(
      ...
      WHERE 
        pish_hikashop_category.category_parent_id = $brand_id AND
        pish_hikashop_category.category_type = 'manufacturer'
      AND pish_hikashop_category.category_parent_id = 10 LIMIT $offfset,$count)as brands
      LEFT JOIN pish_hikashop_file
      ON pish_hikashop_file.file_ref_id = brands.category_id
        AND pish_hikashop_file.file_type = 'category'
      LEFT JOIN pish_hikashop_product
     ON pish_hikashop_product.product_manufacturer_id = brands.category_id
    GROUP BY brands.category_id";
