...
class GetAllCategoryProducts
{
  ...
  {
    $sql = "SELECT new.*,pish_hikashop_product.* FROM(SELECT category_id,category_name from pish_hikashop_category WHERE category_name like '%شیر%' AND category_type = 'product' ORDER BY category_id ASC LIMIT $offset,$count) AS new
  ...
  {
    $sql = "SELECT COUNT(*) as count FROM(SELECT new.*,pish_hikashop_product.* FROM(SELECT category_id,category_name from pish_hikashop_category WHERE category_name like '%شیر%' AND category_type = 'product' ORDER BY category_id ) AS new
    INNER JOIN pish_hikashop_product
  ...
  protected function searchBaseOnBrandName(string $like_brand_name,int $offset, int $count):array {
    $sql = "SELECT * FROM(SELECT new.*,pish_hikashop_product.* FROM(SELECT category_id,category_name from pish_hikashop_category WHERE category_name like '%شیر%' AND category_type = 'manufacturer' ORDER BY category_id ASC LIMIT $offset,$count) AS new
    INNER JOIN pish_hikashop_product
  ...
  protected function searchBaseOnBrandNameCount(string $like_brand_name,int $offset,int $count):int{
    $sql= "SELECT COUNT(*) as count FROM(SELECT new.*,pish_hikashop_product.* FROM(SELECT category_id,category_name from pish_hikashop_category WHERE category_name like '%شیر%' AND category_type = 'manufacturer' ORDER BY category_id) AS new
    INNER JOIN pish_hikashop_product
    ON new.category_id= pish_hikashop_product.product_manufacturer_id)AS result";
      ...
      if($offset!=-1 && $count!=-1 && $search !=-1 && $sarchType !=-1){
        if($sarchType=='category'){
          ...
          if(count($findedProduct)>0){
            ...
            $this->resultJsonEncode(['products'=>$findedProduct,'status'=>true]);
          }
          ...
          if(count($findedProducts)){
            ...
            $this->resultJsonEncode(['products'=>$findedProducts,'status'=>true]);
